...
import es.igosoftware.euclid.experimental.vectorial.rendering.feautures.GEllipseRenderingSymbol;
import es.igosoftware.euclid.experimental.vectorial.rendering.feautures.GPolygonRenderingShape;
import es.igosoftware.euclid.experimental.vectorial.rendering.feautures.GPolygonalChainRenderingShape;
import es.igosoftware.euclid.experimental.vectorial.rendering.feautures.GRenderingSymbol;
...
public abstract class GRenderingStyleAbstract
   ...
   public GPolygonRenderingShape getSurfaceShape(final ISurface2D<?> surface,
      ...
      if (surface instanceof IPolygon2D) {
         ...
         if (polygon instanceof IComplexPolygon2D) {
            ...
            final Area complexShape = rc.toScaledAndTranslatedPoints(complexPolygon.getHull()).asArea();

            for (final ISimplePolygon2D hole : complexPolygon.getHoles()) {
               // complexShape.exclusiveOr(getPoints(hole, scale, region).asArea());
               complexShape.subtract(rc.toScaledAndTranslatedPoints(hole).asArea());
            }
         ...
         return new GPolygonRenderingShape(polygon, rc.toScaledAndTranslatedPoints(polygon).asShape(), surfaceBorderSize, this,
                  rc);
      }
   ...
   public GPolygonalChainRenderingShape getCurveShape(final ICurve2D<?> curve,
      ...
      if (curve instanceof IPolygonalChain2D) {
         ...
         return new GPolygonalChainRenderingShape(polygonalChain, rc.toScaledAndTranslatedPoints(polygonalChain),

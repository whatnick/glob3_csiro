...
public class GSwitchExpression<GeometryT extends IGeometry2D, KeyT, ResultT>
   ...
   public void preRender(final IVectorI2 renderExtent,
      ...
      _keyExpression.preRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      _defaultExpression.preRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      for (final GPair<KeyT, IExpression<GeometryT, ? extends ResultT>> casePair : _casesExpressions) {
         final IExpression<GeometryT, ? extends ResultT> caseExpression = casePair._second;
         caseExpression.preRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      }
   }
   ...
   public void postRender(final IVectorI2 renderExtent,
                          ...
                          final ISymbolizer2D renderingStyle,
                          final IVectorial2DDrawer drawer) {
      _keyExpression.postRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      _defaultExpression.postRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      for (final GPair<KeyT, IExpression<GeometryT, ? extends ResultT>> casePair : _casesExpressions) {
         final IExpression<GeometryT, ? extends ResultT> caseExpression = casePair._second;
         caseExpression.postRender(renderExtent, projectionTool, viewport, renderingStyle, drawer);
      }
   }
